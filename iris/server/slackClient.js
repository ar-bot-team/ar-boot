...
function handleOnAuthenticated (rtmStartData) {
    console.log(`Logged in as a ${rtmStartData.self.name} of team ${rtmStartData.team.name}, but not yet connected to a channel`)
}
...
function handleOnMessage (message) {
    if (message.text.toLowerCase().includes('iris')) {
        nlp.ask(message.text, (err, res) => {
            ...
            if (!res.intent) {
                return rtm.sendMessage('Sorry I dont know what are you talking about.', message.channel)
            } else {
                console.log(res)
                return rtm.sendMessage('Sorry I dont know what are you talking about.', message.channel)
            }
